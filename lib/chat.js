...
async function streamChat(config, messages, onDelta) {
  ...
  if (!res.ok || !res.body) {
    ...
    // Status is attached separately so the route can classify the failure for
    // the user without ever relaying the body, which can echo request material.
    const err = new Error(`Chat ${res.status}: ${detail.slice(0, 300)}`);
